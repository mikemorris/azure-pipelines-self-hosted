...
locals {
  install_windows_agent_path = "scripts/install_azure_pipelines_windows_agent.ps1"
}
...
resource "azurerm_virtual_machine_extension" "install_windows_agent" {
  ...
  "commandToExecute": "powershell.exe -ExecutionPolicy Unrestricted -File ./${azurerm_storage_blob.install_windows_agent_script.name} -URL ${var.url} -TOKEN ${var.token} -POOL ${var.pool} -AGENT ${var.agent}"
  }
SETTINGS
}
